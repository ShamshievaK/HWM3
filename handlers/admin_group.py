...
async def pin_zakrep(message: types.Message):
    if message.chat.type != 'private':
        if not message.reply_to_message:
            await message.answer("Команда /pin должна быть ответом на сообщениею")
        else:
        ...
        await message.answer("Команда должна быть испльзована в груупе")
...
def register_admin_group(dp: Dispatcher):
    ...
    dp.register_message_handler(pin_zakrep, text='!pin')
